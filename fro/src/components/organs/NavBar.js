...
const NavBar = () => {
    ...
    return (
        <header className="w-full h-auto bg-transparent overflow-x-hidden fixed z-50 top-0 left-0">
             <nav className={`w-full lg:h-20 md:h-24 h-20 ${navBarColor ? "bg-zinc-900" : " bg-transparent"} lg:px-16 md:px-9 px-8 flex justify-between items-center`}>
                <Link to={`/`} className="font-extrabold flex items-center relative md:text-2xl text-lg">
                    <img src={MyCustomImage} alt="My Custom Image" className="w-6 h-6 md:w-8 md:h-8 absolute -top-5 md:left-2 left-3" />
                    <span className="text-white">Body</span>
                    <span className="bg-gradient-to-r from-[#00c6ac] to-bodyhack bg-clip-text text-transparent">Hacker</span>
                </Link>
