...
export const loadUsers = ({ page, rowsPerPage }) => async (
  ...
  try {
    var params = getState().interactionType.colsearch;
...
export const SaveFormData = () => async (dispatch, getState) => {
  try {
    ...
    const currentState = getState().interactionType.formData;
    if (currentState.InteractionTypeID) {
      const res = await helper.UpdateServiceWorker({
        url: "interaction-type",
        currentState,
        id: currentState.InteractionTypeID,
      });
      if (res.data.success === true) {
        dispatch(setAlert(true));
        dispatch(setAlertStatus(true));
