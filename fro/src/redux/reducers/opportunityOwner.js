...
export const SaveFormData = () => async (dispatch, getState) => {
  try {
    ...
    const currentState = getState().opportunityOwner.formData;
    if (currentState.OpportunityOwnerID) {
      const res = await helper.UpdateServiceWorker({
        url: "opportunity-owner",
        currentState,
        id: currentState.OpportunityOwnerID,
      });
      if (res.data.success === true) {
        dispatch(setAlert(true));
        dispatch(setAlertStatus(true));
