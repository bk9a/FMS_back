...
const AddEquipment = () => {
  ...
  const getDataById = async (id) => {
    await axios
      .get(`${process.env.REACT_APP_API}/equipment/${id}`, {
        headers: {
  ...
  const sendData = async () => {
    if (id) {
      ...
      await axios
        .put(`${process.env.REACT_APP_API}/apiv1/equipment/${id}`, formData, {
          headers: {
            authorization: "Bearer " + localStorage.getItem("ITSToken"),
          },
        })
        .then((response) => {
          if (response.data.success === true) {
            ...
            alert.error("Хугацаа дууссан байна! Та дахин нэвтэрнэ үү!", {
              closeCopy: "Хаах",
              onOpen: () => {
      ...
      await axios
        .post(`${process.env.REACT_APP_API}/equipment`, formData, {
          headers: {
            authorization: "Bearer " + localStorage.getItem("ITSToken"),
          },
        })
