...
export const EquipmentCols = [
  ...
  },
  {
    ...
    Cell: (props) => {
      return (
        <Link to={`/admin/addequipment/${props.row.original.id}`}>
  ...
  },
  {
    ...
    Cell: (props) => {
      ...
      const deleteNews = async () => {
        ...
        await axios
          .delete(
            `${process.env.REACT_APP_API}/equipment/${props.row.original.id}`, {
              headers: { authorization: "Bearer " + localStorage.getItem("ITSToken") },
            }
          )
          .then((response) => {
