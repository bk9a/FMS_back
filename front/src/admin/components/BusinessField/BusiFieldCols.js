...
export const BusiFieldCols = [
  ...
  { Header: "Name", accessor: "title" },
  {
    ...
    Cell: (props) => {
      let image = null;
      image = `data:image/jpg;base64,${props.row.original.icon_string}`;
  ...
  },
  {
    ...
    Cell: (props) => {
      return (
        <Link to={`/admin/addbusifield/${props.row.original.id}`}>
  ...
  },
  {
    ...
    Cell: (props) => {
      ...
      const deleteNews = async () => {
        ...
        await axios
          .delete(
            `${process.env.REACT_APP_API}/business-field/${props.row.original.id}`,
            {
              headers: {
